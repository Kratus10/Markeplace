...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const validation = retireSchema.safeParse(body);
    ...
    // Validate product can be retired
    if (!['PUBLISHED', 'HIDDEN'].includes(product.status)) {
      return NextResponse.json({ 
        error: 'Only published or hidden products can be retired' 
      }, { status: 400 });
    }
